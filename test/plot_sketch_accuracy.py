...
for epsilon in epsilon_values:
    for n_cli in clients:
        ...
        for iteration in range(N_REP):
            ...
            idx = sorted(range(len(true_counters)), key=true_counters.__getitem__)

            top_k = idx[:len(true_counters)-(k+1):-1]

            true_epsilon = np.append(true_epsilon, float(error[top_k].sum()) / (k*true_counters[top_k].sum()) ) 
...
layout = Layout(
    title='Top 50 items',
    xaxis= XAxis( 
        title= 'Average error over top-50 items',
        type='log',
        autorange=True
        ),
